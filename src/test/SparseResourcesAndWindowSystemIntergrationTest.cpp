...
surfaceFormat2KHR.surfaceFormat = surfaceFormatKHRs[0];//VkSurfaceFormatKHR类型，前面已经描述过这里不再复述
	...
	// WSI Swapchain  参见p3138
	{
		...
		*/
		...
		vkAcquireNextImageKHR(device, swapchainKHR, VK_TIMEOUT, VkSemaphore{/*假设这是一个有效的VkSemaphore*/ }/*为一个触发的semaphore*/, VkFence{/*假设这是一个有效的VkFence*/ }/*为一个触发的fence*/, & imageIndex/*返回下一个可用的image的索引*/);
		/*
		1.swapchain 不能处于retired state
